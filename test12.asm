...
; LCD registers
		...
		_mr_lcd_oled_dimming_enabled, _mr_lcd_oled_dimming_count
		_mr_lcd_module_state					; <0> Display On/Off (1/0), <1> Power On/Off (1/0)
	endc
...
ISR     org     0x004
	...
	incf   _mr_screen_buffer_update, F

	; Update dimming count
	banksel	_mr_lcd_oled_dimming_count
	movf	_mr_lcd_oled_dimming_count, F
	btfsc	STATUS, Z
		goto	ISR_TIMER2_RESET
ISR_TIMER2_DEC
	banksel	_mr_lcd_oled_dimming_count
	decf	_mr_lcd_oled_dimming_count, F
	goto	ISR_exit
ISR_TIMER2_RESET
	banksel	_mr_lcd_oled_dimming_enabled
	movf	_mr_lcd_oled_dimming_enabled, W
	movwf	_mr_lcd_oled_dimming_count
	goto	ISR_exit
...
main_loop_set_dim_state
	movf	_mr_lcd_oled_dimming_count, F
	btfsc	STATUS, Z
		goto	main_loop_set_dim_state_display_enabled

main_loop_set_dim_state_display_disabled
	movlw	(LCD_BASE_ADDR>>8)			; Select high address
	movwf	PCLATH					; For the next 'call'
	call	LCD_CTRL_DISPLAY_OFF
	clrf	PCLATH
	goto	main_loop_screen_update
main_loop_set_dim_state_display_enabled
	movlw	(LCD_BASE_ADDR>>8)			; Select high address
	movwf	PCLATH					; For the next 'call'
	call	LCD_CTRL_DISPLAY_ON
	clrf	PCLATH
	goto	main_loop_screen_update

main_loop_screen_update
	banksel	_mr_screen_buffer_update
...
main_loop_button_up
	...
	sublw	CMD_UP
	btfss	STATUS, Z
		goto	main_loop_button_down
	banksel	_mr_lcd_oled_dimming_enabled
	incf	_mr_lcd_oled_dimming_enabled, F
	goto	main_loop_repeat
main_loop_button_down
	movf	_mr_cmd_cur, W
	sublw	CMD_DOWN
	btfss	STATUS, Z
		goto	main_loop_repeat
	banksel	_mr_lcd_oled_dimming_enabled
	decf	_mr_lcd_oled_dimming_enabled, F
	goto	main_loop_repeat
...
;	movlw	MODE_POWEROFF					; Set the current mode
	...
	clrf	_mr_lcd_oled_dimming_enabled
	clrf	_mr_lcd_oled_dimming_count
